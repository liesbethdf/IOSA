...
\begin{figure}[!t]
	...
	\caption{\label{Eskom-electricity-prices} The price of electricity in South Africa in .}
\end{figure}
...
CPI's projections for coal export revenues towards 2035, i.e. towards the end of the period under study, are 90\% lower under 2DS than under BAU. They project that the coal sector, thanks to sustained local demand mainly from electricity production, can support about 30 000 jobs on the long term.% towards the end of the period under consideration.
The lower revenue is due both to lower export volumes and lower prices. 

To make an estimate from a macroeconomic point of view of the effect on the economy of the difference in coal export demand in 2035 under 2DS versus BAU, the demand-pull IO-model is used provided by eqn. \ref{IOdemand-driven}. %using the macroeconomic picture of the economy provided by the IO-model, of the effect on employment of the difference in coal export demand in 2035 under 2DS versus BAU, a comparison is made between the South African economy in the year 2014 and the same economy with the sole difference that coal exports are diminished by 90\%, following CPI's projections. 
...
The prospective price and volume scenarios of coal export under a 2DS provided by CPI, and used here as input, already include the impact on the profitability of the sector of the lower coal-price on the international market. A number of unprofitable mines are assumed closed in the 2DS scenario. 

The demand-pull model as given by eqn. \ref{IOdemand-driven} assumes that prices are constant. To be able to use the model to determine the impact of different export volumes under BAU versus 2DS on output and employment per sector throughout the economy, the export volumes under both scenarios are evaluated under the BAU price.


The volume and the price change under BAU versus 2DS are treated separate instances. The volume impact is taken into account by determining output and resulting employment per sector. To this end, the IO-model is applied to a value of exports calculated as if the price is the BAU price. Then, total revenue and gross operating surplus are recalculated 


The difference between the BAU and the 2DS scenarios in 2035 is approximated by using for the BAU demand scenario, demand as in the South-african economy in the year 2014, as provided by the IOT. As demand scenario for 2DS in 2035, the same as BAU is used with the sole difference that coal exports are diminished by 90\%, following CPI's projections. The impact on employment between the two scenarios can be found in table \ref{coalExport_BAUvs2DS_2035}. The top part of the table shows that the number of jobs in the coal sector under 2DS is about half that under BAU. The IO-model projects about 39 600 jobs in the coal sector under 2DS, which is in the same ballpark as CPI's estimate. Of this total number of jobs in coal mining, 15\% is generated by export and some consumption by households. The biggest share, 85\%, is generated by demand from other sectors. 
...
jobs are lost throughout the economy, in addition to the job losses in coal-mining itself. The Transport sector is the first impacted because of the lost activity from transporting coal by rail. 
...
Due to the loss of cross-subsidies from coal-exports, the domestic price of coal might increase. 
