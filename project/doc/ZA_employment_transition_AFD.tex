...
\begin{equation}
(\mathbb{I} - \tilde{A}^m)^{-1} - \mathbb{I} = \sum_{k=1}^{+\infty}  (\tilde{A}^m)^k  
\end{equation}
Whereas the significance of $A^m D^d$ is quite clear --- it determines the imported intermediate inputs needed to satisfy domestically produced final demand, the meaning of the following terms $(A^m)^k D^d$ with $k>1$ seems nebulous. To resolve this issue, we determine the relationship between $\tilde{A}^m$ and $A^m$, where the latter is the technical coefficients matrix for imported goods, defined as usual.
